...
class PageExcerptMulti extends WP_Widget {
	...
	function PageExcerptMulti() {
		$widget_ops = array( 'classname' => 'page_widget_excerpt_multi', 'description' => __( 'Page Excerpt', 'pxls' ) );
		$this->WP_Widget( 'pageexcerptmulti', __( 'Page Excerpt', 'pxls' ), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? __( 'Excerpt', 'pxls'  ) : $instance['title']);
		...
		if (function_exists('the_excerpt_reloaded')) 
			...
			echo '<p class="more" title="Continue reading '.$title.'"><a href="'.get_permalink($instance['page_ID']).'">'.$instance['more_text'].'</a></p>'; // 'more' link
		}
		endwhile;
		?>
		</ul>
	...
	?>  
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'pxls' ); ?></label> 
...
class PostExcerptMulti extends WP_Widget {
	...
	function PostExcerptMulti() {
		$widget_ops = array( 'classname' => 'post_widget_excerpt_multi', 'description' => __( 'Post Excerpt', 'pxls' ) );
		$this->WP_Widget( 'postexcerptmulti', __( 'Post Excerpt', 'pxls' ), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		if ( function_exists( 'the_excerpt_reloaded' ) ) 
			...
			the_excerpt();  // this covers Advanced Excerpt as well as the built-in one
			echo '<p class="more" title="Continue reading ' . $title . '"><a href="' . get_permalink( $instance['post_ID'] ) . '">' . $instance['more_text'] . '</a></p>'; // 'more' link
		}
		endwhile;
		?>
		</ul>
